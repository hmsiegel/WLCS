...
internal sealed class ProcessInboxMessagesJob : IJob
{
    ...
    {
        List<InboxMessageResponse> inboxMessagesList = await GetInboxMessagesAsync().ConfigureAwait(false);
        ...
        {
            IIntegrationEvent integrationEvent = JsonConvert.DeserializeObject<IIntegrationEvent>(inboxMessage.Content, _jsonSerializerSettings)!;
            ...
            PolicyResult result = await _policy.ExecuteAndCaptureAsync(async () =>
            {
                ...
                {
                    await handler.Handle(integrationEvent, context.CancellationToken).ConfigureAwait(false);
                }
            }).ConfigureAwait(false);

            await UpdateInboxMessageAsync(inboxMessage, result.FinalException).ConfigureAwait(false);
        }
    }
    ...
    private async Task<List<InboxMessageResponse>> GetInboxMessagesAsync()
    {
        ...
        IEnumerable<InboxMessageResponse> inboxMessages = await _sqlQueryExecutor.QueryAsync<InboxMessageResponse>(getInboxMessagesSql).ConfigureAwait(false);
